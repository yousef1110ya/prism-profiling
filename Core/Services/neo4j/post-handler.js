...
async function upsertPost(postId, postData) {
  const { id, text, group_id, media, privacy, created_at, user } = postData;
...
export async function createPost(postData) {
  ...
  const postType =
    media.length === 1 && media[0].type === "video" ? "REEL" : "Post";
  const hashtags = extractHashtags(text);
  const mediaJson = JSON.stringify(media);
  if (postType === "Post" && privacy === "public" && user.is_private === 0) {
    // if these 2 are true then save that post to the elasticsearch cluster
  ...
  try {
    ...
    console.log(
      `${postType} ${id} created with media as JSON and ${hashtags.length} hashtag(s).`,
    );
  } catch (error) {
...
export async function unlikePost(userId, postId) {
  ...
  try {
    ...
    if (deletedCount === 1) {
      ...
      if (Count === 1) {
        console.log(
          "deleted the like relationship for a REEL !!  and all is fun and games ",
        );
      }
    }
